...
class UcoaAccessibilityService : AccessibilityService() {
    ...
    override fun onServiceConnected() {
        ...
        // No always-on-top execution window. Live execution is rendered inside the conversation UI.
        remoteBridge = RemoteCommandBridge(this, this).also { it.start() }
    }

    override fun onDestroy() {
        remoteBridge?.stop(); remoteBridge = null
    ...
    override fun onAccessibilityEvent(event: AccessibilityEvent?) {
        event?.packageName?.toString()?.takeIf { it.isNotBlank() }?.let {
            if (it != lastForegroundPackage) { lastForegroundPackage = it; UcoaDiagnostics.log("OBSERVE", "تغير التطبيق الأمامي", "package=$it event=${event.eventType}") }
        }
    }
    override fun onInterrupt() { UcoaDiagnostics.log("ACCESSIBILITY", "أرسل النظام interrupt للخدمة") }

    fun foregroundPackageName(): String? = lastForegroundPackage ?: rootInActiveWindow?.packageName?.toString() ?: windows.asSequence().mapNotNull { it.root?.packageName?.toString() }.firstOrNull()
    fun findText(text: String): AccessibilityNodeInfo? = allNodes().firstOrNull { nodeText(it).contains(text, ignoreCase = true) }
    ...
    fun typeText(text: String): Boolean { val node = allNodes().firstOrNull { it.isEditable && it.isFocused } ?: allNodes().firstOrNull { it.isEditable } ?: rootInActiveWindow?.findFocus(AccessibilityNodeInfo.FOCUS_INPUT) ?: return false; return setNodeText(node, text) }
    ...
    fun packageForName(query: String): String? {
        ...
        val known = mapOf("capcut" to "com.lemon.lvoverseas", "كاب كات" to "com.lemon.lvoverseas", "كابكات" to "com.lemon.lvoverseas", "youtube" to "com.google.android.youtube", "يوتيوب" to "com.google.android.youtube", "canva" to "com.canva.editor", "كانفا" to "com.canva.editor", "chrome" to "com.android.chrome", "كروم" to "com.android.chrome", "instagram" to "com.instagram.android", "انستجرام" to "com.instagram.android", "انستغرام" to "com.instagram.android", "whatsapp" to "com.whatsapp", "واتساب" to "com.whatsapp", "telegram" to "org.telegram.messenger", "تليجرام" to "org.telegram.messenger", "settings" to "com.android.settings", "الإعدادات" to "com.android.settings", "اعدادات" to "com.android.settings", "الضبط" to "com.android.settings")
        return known.entries.firstOrNull { q.contains(normalize(it.key)) }?.value
    }
    fun openApp(pkg: String): Boolean = try { val intent = packageManager.getLaunchIntentForPackage(pkg) ?: return false.also { UcoaDiagnostics.log("EXECUTOR", "لم نجد launch intent", "package=$pkg") }; intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK); startActivity(intent); UcoaDiagnostics.log("EXECUTOR", "تم استدعاء startActivity", "package=$pkg"); true } catch (e: Exception) { UcoaDiagnostics.log("EXECUTOR", "استثناء أثناء فتح التطبيق", "package=$pkg error=${e.javaClass.simpleName}: ${e.message}"); false }
    fun openAppByName(query: String): Boolean {
        val q = normalize(query); val direct = packageForName(query); if (direct != null && openApp(direct)) return true
        val pm = packageManager; val candidates = pm.getInstalledApplications(0).filter { !it.packageName.equals(packageName, true) }.mapNotNull { app -> val label = pm.getApplicationLabel(app)?.toString() ?: return@mapNotNull null; if (normalize(label).contains(q) || normalize(app.packageName).contains(q)) app else null }.sortedBy { pm.getApplicationLabel(it)?.toString()?.length ?: 999 }
        val fallback = candidates.firstOrNull()?.packageName; UcoaDiagnostics.log("EXECUTOR", "نتيجة مطابقة اسم التطبيق", "query=$query mapped=$direct fallback=$fallback"); return fallback?.let { openApp(it) } ?: false
    }
    ...
    private fun accessibilityRoots(): List<AccessibilityNodeInfo> { val roots = mutableListOf<AccessibilityNodeInfo>(); rootInActiveWindow?.let { roots += it }; windows.asSequence().mapNotNull { it.root }.forEach { root -> if (roots.none { it.windowId == root.windowId }) roots += root }; return roots }
    private fun allNodes(max: Int = 400): List<AccessibilityNodeInfo> { val result = mutableListOf<AccessibilityNodeInfo>(); fun walk(n: AccessibilityNodeInfo?) { if (n == null || result.size >= max) return; result += n; for (i in 0 until n.childCount) walk(n.getChild(i)) }; accessibilityRoots().forEach(::walk); return result }
    ...
    private fun gesture(x1: Float, y1: Float, x2: Float, y2: Float, d: Long): Boolean { val p = Path().apply { moveTo(x1, y1); lineTo(x2, y2) }; return dispatchGesture(GestureDescription.Builder().addStroke(GestureDescription.StrokeDescription(p, 0, d.coerceAtLeast(1))).build(), null, null) }
}
