...
class RemoteCommandBridge(private val context: Context, private val service: UcoaAccessibilityService) {
    ...
    private fun executeCommandInternal(command: JSONObject) {
        ...
        for (step in 0 until 24) {
            ...
            val targetConfirmedByUcoa = verificationRequired && verification.first && taskRequestsSettings(task) && afterUi.isNotBlank() && afterUi != "[]" && !afterShot.isNullOrBlank(); if (targetConfirmedByUcoa) { completed = true; lastError = ""; LiveExecutionState.update(task, "نجاح مؤكد من UCOA", "Android Settings", step + 1, 24, true, afterShot, screenWidth(), screenHeight(), actionTapX(action), actionTapY(action)) }
            ...
            if (completed || name == "done" || action.optBoolean("done", false)) { completed = completed || (actionOk && verified); if (!completed && lastError.isBlank()) lastError = verification.third ?: "لم يثبت UCOA اكتمال المهمة"; break }; if (!actionOk && !action.optBoolean("optional", false)) { lastError = action.optString("error", "action failed"); break }
        }
