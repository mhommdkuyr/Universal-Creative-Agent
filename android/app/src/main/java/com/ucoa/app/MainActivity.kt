...
class MainActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        unsubscribeDiagnostics = UcoaDiagnostics.subscribe { event ->
            runOnUiThread { if (event.stage == "REMOTE_BRIDGE" || event.stage == "AGENT" || event.stage == "EXECUTOR") status.text = event.message }
        }
        refreshConnectionState()
        intent.getStringExtra("smoke_task")?.trim()?.takeIf { it.isNotEmpty() }?.let { task ->
            window.decorView.postDelayed({ input.setText(task); analyzeTask() }, 900L)
        }
    }
    ...
    private fun buildUi(): View {
        ...
        top.addView(menuButton, LinearLayout.LayoutParams(54, 54))
        main.addView(top)

        status = TextView(this).apply { textSize = 11f; setTextColor(0xFF9AA3AE.toInt()); gravity = Gravity.CENTER; setPadding(8, 2, 8, 4) }
        main.addView(status, LinearLayout.LayoutParams(-1, 28))
        ...
        chat = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; gravity = Gravity.CENTER_HORIZONTAL; setPadding(16, 12, 16, 22) }
        addAssistantBubble("اكتب ما تريد، وسأخطط له سحابيًا ثم أنفذه مرة واحدة مع تحقق حي من النتيجة.")
        ...
        composer.addView(iconButton("＋", "إضافة ملف") { chooseMedia() }, LinearLayout.LayoutParams(44, 52))
        composer.addView(iconButton("⌕", "الصوت") { startSpeech() }, LinearLayout.LayoutParams(44, 52))
        ...
        composer.addView(iconButton("↑", "إرسال") { analyzeTask() }, LinearLayout.LayoutParams(50, 52))
        main.addView(composer, LinearLayout.LayoutParams(-1, 78))
        root.addView(main)

        drawerScrim = View(this).apply { setBackgroundColor(0x99000000.toInt()); visibility = View.GONE; setOnClickListener { toggleDrawer(false) } }
        root.addView(drawerScrim, FrameLayout.LayoutParams(-1, -1))
        drawer = buildDrawer()
        root.addView(drawer, FrameLayout.LayoutParams((resources.displayMetrics.widthPixels * 0.74f).toInt(), -1, Gravity.END))
        return root
    }
    ...
    private fun buildDrawer(): LinearLayout {
        ...
        panel.addView(drawerAction("＋  محادثة جديدة") { startNewConversation(); toggleDrawer(false) })
        panel.addView(drawerAction("⌕  البحث في المحادثات") { showSearchDialog() })
        panel.addView(drawerAction("▣  التطبيقات المتصلة") { showConnectedApps() })
        panel.addView(Space(this), LinearLayout.LayoutParams(1, 24))
        panel.addView(View(this).apply { setBackgroundColor(0xFF2A2D31.toInt()) }, LinearLayout.LayoutParams(-1, 1))
        val list = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(0, 16, 0, 16) }
        listOf("محادثة جديدة", "اختبار تنفيذ الأوامر", "تصميم واجهة UCOA", "ربط الهاتف بالسحابة", "مراجعة التطبيق", "اختبارات Android").forEach { title ->
            list.addView(TextView(this).apply { text = title; textSize = 15f; setTextColor(0xFFD9DDE3.toInt()); gravity = Gravity.RIGHT or Gravity.CENTER_VERTICAL; setPadding(14, 16, 14, 16); setOnClickListener { currentConversationTitle = title; toggleDrawer(false); addAssistantBubble("تم فتح «$title». تفاصيل المحادثة محفوظة في هذه الجلسة.") } }, LinearLayout.LayoutParams(-1, 52))
        }
        panel.addView(ScrollView(this).apply { isFillViewport = true; addView(list) }, LinearLayout.LayoutParams(-1, 0, 1f))
        panel.addView(drawerAction("⚙  الإعدادات") { showSettingsPage() })
        return panel
    }
    ...
    private fun startNewConversation() {
        conversationStarted = false; currentConversationTitle = "محادثة جديدة"; latestTaskText = ""; selectedMedia.clear(); LiveExecutionState.finish(false, "تم فتح محادثة جديدة")
        chat.removeAllViews(); addAssistantBubble("محادثة جديدة. هذه المساحة للمحادثة والتخطيط؛ التنفيذ يبدأ فقط بعد اعتماد المهمة.")
        menuButton.text = "◌"; menuButton.contentDescription = "محادثة جديدة"
    }
    ...
    private fun showSettingsPage() {
        val page = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(24); setBackgroundColor(0xFF101114.toInt()) }
        ...
        page.addView(settingRow("عقل AI السحابي", "المزودات والنماذج سحابية فقط") { showBrainSettings() })
        page.addView(settingRow("الوصول والتنفيذ", "Accessibility + مراقبة الحالة") { connectPhone() })
        page.addView(settingRow("المظهر", "الوضع الداكن، اللغة، كثافة الواجهة") { Toast.makeText(this, "إعدادات المظهر جاهزة للتخصيص.", Toast.LENGTH_SHORT).show() })
        page.addView(settingRow("الخصوصية", "السجل، لقطات الشاشة، القياس") { Toast.makeText(this, "البيانات الحساسة لا تظهر في سجل التشخيص.", Toast.LENGTH_SHORT).show() })
        page.addView(settingRow("التشخيص", "عرض سجل التنفيذ") { showDiagnostics() })
        page.addView(Button(this).apply { text = "إغلاق"; setOnClickListener { setContentView(buildUi()) } }, LinearLayout.LayoutParams(-1, 54).apply { setMargins(0, 20, 0, 0) })
        setContentView(page)
    }

    private fun settingRow(titleText: String, sub: String, click: () -> Unit): View = LinearLayout(this).apply {
        orientation = LinearLayout.VERTICAL; setPadding(16, 14, 16, 14); background = rounded(0xFF1D1F23.toInt(), 18f); setOnClickListener { click() }
        addView(TextView(this@MainActivity).apply { text = titleText; textSize = 17f; typeface = Typeface.DEFAULT_BOLD; setTextColor(Color.WHITE) })
        addView(TextView(this@MainActivity).apply { text = sub; textSize = 12f; setTextColor(0xFF9EA5AF.toInt()); setPadding(0, 5, 0, 0) })
        layoutParams = LinearLayout.LayoutParams(-1, 78).apply { setMargins(0, 0, 0, 10) }
    }

    private fun renderLive(s: LiveExecutionState.Snapshot) {
        if (!s.active && s.task.isBlank()) return
        if (liveCard == null) {
            liveCard = LiveExecutionCard(this)
            chat.addView(liveCard, LinearLayout.LayoutParams(-1, 430).apply { setMargins(0, 8, 0, 18) })
        }
        liveCard?.render(s)
    }
    ...
    private fun refreshConnectionState() {
        ...
        status.text = when { live -> "● متصل — تنفيذ ومراقبة سحابية"; enabled -> "● الصلاحية مفعلة — الخدمة قيد الاتصال"; else -> "○ فعّل الوصول من الإعدادات لبدء التنفيذ" }
    }
    ...
    private fun addPlanCard(summary: String, steps: List<String>) {
        ...
        card.addView(Button(this).apply {
            ...
            setOnClickListener {
                ...
                LiveExecutionState.begin(latestTaskText); addAssistantBubble("بدأ التنفيذ. لن يُعتبر أي نقر نجاحًا قبل التحقق.")
                UniversalAgentLoop(brain).start(latestTaskText + "\nالخطة: " + steps.mapIndexed { i, x -> "${i + 1}. $x" }.joinToString("\n"), object : UniversalAgentLoop.Listener {
                    override fun onEvent(text: String) { runOnUiThread { status.text = text.take(180) } }
                    override fun onFinished(success: Boolean) { runOnUiThread { addAssistantBubble(if (success) "✓ اكتملت المهمة بعد التحقق." else "⚠ توقفت المهمة ولم يثبت اكتمالها.") } }
    ...
    private fun showBrainSettings() {
        ...
        val endpoint = EditText(this).apply { hint = "عنوان Cloud Brain"; setSingleLine(); setText(brain.endpoint()); setTextColor(Color.WHITE); hintTextColor = 0xFF777C84.toInt() }
        val token = EditText(this).apply { hint = "رمز الوصول"; setSingleLine(); setText(brain.token()); setTextColor(Color.WHITE); inputType = 0x81 }
        box.addView(endpoint); box.addView(token)
        AlertDialog.Builder(this).setTitle("ربط Cloud Brain").setView(box).setNegativeButton("إلغاء", null).setPositiveButton("حفظ") { _, _ -> brain.saveConfig(endpoint.text.toString(), token.text.toString()); refreshConnectionState() }.setNeutralButton("اختبار") { _, _ -> brain.health { ok, detail -> runOnUiThread { Toast.makeText(this, if (ok) "Cloud Brain متاح: $detail" else "فشل: $detail", Toast.LENGTH_LONG).show() } } }.show()
    }
    ...
    private class LiveExecutionCard(context: Context) : FrameLayout(context) {
        ...
        private val phase = TextView(context)
