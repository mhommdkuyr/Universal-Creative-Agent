...
class MainActivity : Activity() {
    ...
    private fun refreshConnectionState() {
        ...
        val brainText = if (brain.configured()) " • عقل AI: جاهز" else " • عقل AI: غير مُعد"
        ...
        connectButton.isEnabled = !live || enabled
    }

    private fun showBrainSettings() {
        val box = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(18, 8, 18, 4) }
        val endpoint = EditText(this).apply { hint = "عنوان Brain API، مثال: https://your-server"; setSingleLine(); setText(brain.endpoint()) }
        val token = EditText(this).apply { hint = "رمز الوصول (اختياري إذا كان الخادم محميًا)"; setSingleLine(); setText(brain.token()); inputType = 0x81 }
        box.addView(endpoint); box.addView(token)
        AlertDialog.Builder(this).setTitle("ربط عقل AI العالمي").setView(box)
            .setNegativeButton("إلغاء", null)
            .setNeutralButton("اختبار الاتصال") { _, _ ->
                brain.saveConfig(endpoint.text.toString(), token.text.toString())
                status.text = "يجري اختبار الاتصال…"
                brain.plan("اختبار اتصال فقط؛ لا تنفذ شيئًا على الهاتف.", emptyList()) { r -> runOnUiThread {
                    status.text = if (r.ok) "● تم الاتصال بعقل AI" else "○ فشل الاتصال: ${r.error ?: "استجابة غير صالحة"}"
                } }
            }
            .setPositiveButton("حفظ", null).show()
    }
    ...
    private fun connectPhone() {
        ...
        Toast.makeText(this, "فعّل Universal Creative Agent ثم ارجع إلى التطبيق.", Toast.LENGTH_LONG).show()
    }
    ...
    private fun startSpeech() {
        try { startActivityForResult(Intent(RecognizerIntent.ACTION_RECOGNIZE_SPEECH).apply { putExtra(RecognizerIntent.EXTRA_LANGUAGE_MODEL, RecognizerIntent.LANGUAGE_MODEL_FREE_FORM); putExtra(RecognizerIntent.EXTRA_LANGUAGE, "ar-SA"); putExtra(RecognizerIntent.EXTRA_PROMPT, "تحدث بطلبك") }, speech) }
    ...
    private fun analyzeTask() {
        ...
        input.setText("")
        val fallback = TaskInterpreter().analyze(latestTaskText, selectedMedia)
        latestPlan = fallback
        addAssistantBubble(if (brain.configured()) "أرسل المهمة إلى عقل AI لبناء الخطة العالمية…" else "عقل AI غير مُعد؛ سأعرض خطة محلية احتياطية ويمكنك تنفيذ دورة الوكيل بعد إعداد العقل.")
        if (brain.configured()) {
            brain.plan(latestTaskText, selectedMedia) { response -> runOnUiThread {
                val p = response.body
                ...
                } else addAssistantBubble("تعذر بناء الخطة من العقل الآن، سأستخدم الخطة الاحتياطية: ${response.error ?: "خطأ غير معروف"}")
    ...
    private fun addPlanCard(plan: TaskInterpreter.PlanResult) {
        latestPlan = plan
        latestPlanCard?.let { chat.removeView(it) }
        ...
        chat.addView(card, LinearLayout.LayoutParams(-1, ViewGroup.LayoutParams.WRAP_CONTENT).apply { setMargins(0, 0, 0, 16) }); latestPlanCard = card
    }
    ...
    private fun showReview(plan: TaskInterpreter.PlanResult) {
        ...
        AlertDialog.Builder(this).setTitle("تعديل الخطة العالمية").setView(editor).setNegativeButton("إلغاء", null).setPositiveButton("حفظ") { _, _ ->
            val steps = editor.text.toString().lines().map { it.trim() }.filter { it.isNotEmpty() }.map { it.replaceFirst(Regex("^\\d+\\.\\s*"), "") }
            latestPlan = plan.copy(steps = steps); addPlanCard(latestPlan!!); addAssistantBubble("تم حفظ الخطة. سيستخدم الوكيل هذه النتيجة كهدف التنفيذ.")
        }.show()
    }
    ...
    private fun executePlan(card: View) {
        ...
        addAssistantBubble("بدأ الوكيل العالمي. لن يعتمد على اسم تطبيق محدد؛ سيقرأ الشاشة، يقرر الفعل التالي، ينفذه، ثم يعيد المراقبة حتى الإتمام أو الفشل الآمن.")
        UniversalAgentLoop(brain).start(taskForAgent, object : UniversalAgentLoop.Listener {
            ...
            override fun onFinished(success: Boolean) { runOnUiThread { card.isEnabled = true; addAssistantBubble(if (success) "✅ انتهت المهمة وقرر العقل أن النتيجة مكتملة." else "⚠️ توقفت الدورة قبل إعلان الاكتمال. راجع آخر حالة ظهرت في السجل.") } }
        })
    }
}
