...
class AgentBrainClient(private val context: Context) {
    ...
    fun step(task: String, step: Int, history: JSONArray, uiTree: String, screenshotBase64: String?, installedApps: List<String>, attachments: List<String>, approvedRisks: Boolean = false, callback: (Response) -> Unit) {
        val payload = JSONObject().apply {
            put("task", task); put("step", step); put("max_steps", remoteInt("max_steps", 60)); put("history", history); put("ui_tree", uiTree)
        ...
        telemetry("step_requested", JSONObject().put("step", step))
    ...
    private fun ensureClientSession(): JSONObject {
        if (token().isNotBlank()) return JSONObject().put("reused", true)
        val payload = JSONObject().apply {
            put("install_id", installId()); put("app_version", appVersion); put("platform", "android")
            put("device", JSONObject().apply { put("manufacturer", Build.MANUFACTURER); put("model", Build.MODEL); put("android", Build.VERSION.SDK_INT) })
        }
        val body = requestJson("POST", endpoint() + "/v1/client/session", payload, 15000)
        val sessionToken = body.optString("session_token")
        if (sessionToken.isBlank()) throw IllegalStateException("Cloud did not issue a client session")
