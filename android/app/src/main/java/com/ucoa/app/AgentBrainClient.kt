...
class AgentBrainClient(private val context: Context) {
    ...
    fun step(task: String, step: Int, history: JSONArray, uiTree: String, screenshotBase64: String?, callback: (Response) -> Unit) {
        val payload = JSONObject().apply {
            put("task", task)
            put("step", step)
            put("max_steps", 60)
    ...
    private fun post(path: String, payload: JSONObject, callback: (Response) -> Unit) {
        ...
        executor.execute {
            ...
            try {
                ...
                callback(Response(code in 200..299, body, if (code in 200..299) null else "HTTP $code: $text"))
            } catch (e: Exception) {
