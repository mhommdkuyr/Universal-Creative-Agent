...
import app_v3
from remote_ops import _hash, _master_ok, _pg_conn, _require_client
...
def _ensure_schema() -> None:
        ...
        try:
            with conn:
                with conn.cursor() as cur:
                    cur.execute(
                        "CREATE TABLE IF NOT EXISTS ucoa_device_commands (id TEXT PRIMARY KEY, install_id TEXT NOT NULL, kind TEXT NOT NULL, payload JSONB NOT NULL DEFAULT '{}'::jsonb, status TEXT NOT NULL DEFAULT 'queued', created_at TIMESTAMPTZ NOT NULL DEFAULT now(), claimed_at TIMESTAMPTZ, completed_at TIMESTAMPTZ, result JSONB NOT NULL DEFAULT '{}'::jsonb)"
                    )
    ...
    try:
        conn.execute(
            "CREATE TABLE IF NOT EXISTS device_commands(id TEXT PRIMARY KEY, install_id TEXT NOT NULL, kind TEXT NOT NULL, payload TEXT NOT NULL, status TEXT NOT NULL DEFAULT 'queued', created_at REAL NOT NULL, claimed_at REAL, completed_at REAL, result TEXT NOT NULL)"
        )
...
def queue_device_command(install_id: str, req: DeviceCommandRequest, authorization: str | None = Header(default=None)):
    ...
    if conn is not None:
        try:
            with conn:
                with conn.cursor() as cur:
                    cur.execute(
                        "INSERT INTO ucoa_device_commands(id,install_id,kind,payload,created_at,result) VALUES(%s,%s,%s,%s,now(),'{}')",
                        (command_id, install_id, req.kind, json.dumps(payload, ensure_ascii=False)),
                    )
            conn.close()
    ...
    try:
        conn.execute(
            "INSERT INTO device_commands(id,install_id,kind,payload,status,created_at,claimed_at,completed_at,result) VALUES(?,?,?,?,?,?,?,?,?)",
            (command_id, install_id, req.kind, json.dumps(payload, ensure_ascii=False), "queued", time.time(), None, None, "{}"),
        )
        conn.commit()
    finally:
        conn.close()
...
def next_device_command(authorization: str | None = Header(default=None)):
    ...
    if not install_id:
        raise HTTPException(403, "Client session required")
    _ensure_schema(); _requeue_stale()
    conn = _pg_conn()
    if conn is not None:
        try:
            with conn:
                with conn.cursor() as cur:
                    cur.execute(
                        "SELECT id,kind,payload,created_at FROM ucoa_device_commands WHERE install_id=%s AND status='queued' AND created_at > now() - (%s || ' seconds')::interval ORDER BY created_at ASC LIMIT 1 FOR UPDATE SKIP LOCKED",
                        (install_id, COMMAND_TTL),
                    )
                    row = cur.fetchone()
                    if not row:
            ...
            conn.close()
            return {"ok": True, "command": {"id": row[0], "kind": row[1], "payload": row[2], "created_at": row[3].isoformat()}}
    ...
    try:
        ...
        row = conn.execute(
            "SELECT id,kind,payload,created_at FROM device_commands WHERE install_id=? AND status='queued' AND created_at > ? ORDER BY created_at ASC LIMIT 1",
            (install_id, time.time() - COMMAND_TTL),
        ).fetchone()
        if not row:
            conn.commit()
            return {"ok": True, "command": None}
        conn.execute("UPDATE device_commands SET status='claimed',claimed_at=? WHERE id=?", (time.time(), row[0]))
        conn.commit()
...
def complete_device_command(command_id: str, req: DeviceCommandResult, authorization: str | None = Header(default=None)):
    install_id, _ = _require_client(authorization)
    if not install_id or install_id != req.install_id:
        raise HTTPException(403, "install_id mismatch")
    status = req.status.strip().lower()
    if status not in {"completed", "failed", "cancelled"}:
        raise HTTPException(400, "Invalid command status")
    _ensure_schema(); result = json.dumps(req.result, ensure_ascii=False)[:50000]
    conn = _pg_conn()
    if conn is not None:
        try:
            with conn:
    ...
    try:
        cur = conn.execute(
            "UPDATE device_commands SET status=?,completed_at=?,result=? WHERE id=? AND install_id=?",
            (status, time.time(), result, command_id, install_id),
        )
        conn.commit()
        if cur.rowcount != 1:
            raise HTTPException(404, "Command not found")
...
def inspect_device_command(install_id: str, command_id: str, authorization: str | None = Header(default=None)):
    ...
    if conn is not None:
        try:
            with conn:
                with conn.cursor() as cur:
                    cur.execute("SELECT id,install_id,kind,payload,status,created_at,claimed_at,completed_at,result FROM ucoa_device_commands WHERE id=%s AND install_id=%s", (command_id, install_id))
                    row = cur.fetchone()
            conn.close()
            if not row:
                raise HTTPException(404, "Command not found")
