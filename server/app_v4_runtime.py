...
class _DDG(HTMLParser):
    ...
    def handle_endtag(self, tag):
        ...
        if self.t and self.h:
            self.results.append({"title":self.t.strip(),"url":self.h,"snippet":self.s.strip()})
            self.t=self.h=self.s=""
...
def _requested_app(task: str, installed: list[str]) -> str | None:
    t=task.lower()
    aliases={"capcut":"CapCut","كاب كات":"CapCut","youtube":"YouTube","يوتيوب":"YouTube","canva":"Canva","كانفا":"Canva","chrome":"Chrome","كروم":"Chrome","instagram":"Instagram","انستجرام":"Instagram","whatsapp":"WhatsApp","واتساب":"WhatsApp"}
...
def run_plan(req: Any) -> dict[str, Any]:
    ...
    payload={"task":req.task,"attachments":req.attachments[:8],"device":req.device,"memory":app_v3.memory(sid,12)}
    ...
    try:
        ...
        result={"summary":str(x.get("summary","UCOA plan")),"steps":[str(s) for s in steps[:6]],"provider":"huggingface-qwen3-vl-235b","output_mode":"primary_model"}
    except Exception as exc:
        result={"summary":"خطة آمنة احتياطية","steps":["افتح التطبيق الهدف.","نفذ الإجراء المطلوب.","تحقق من النتيجة."],"provider":"repair","output_mode":"repair","error":str(exc)}
...
def run_step(req: Any) -> dict[str, Any]:
    ...
    result["session_id"]=sid; result["research"]=research_results; result["verification"]=app_v3.safety(req.task,result,req.approved_risks)
