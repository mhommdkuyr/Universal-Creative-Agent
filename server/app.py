...
app = FastAPI(title="UCOA Universal Agent Brain", version="1.3.0")
...
def model_call(system: str, user: str, image_b64: str | None = None) -> str:
    ...
    try:
        with urlopen(Request(url, data=json.dumps(payload).encode(), headers=headers, method="POST"), timeout=int(os.getenv("UCOA_MODEL_TIMEOUT", "300"))) as response:
