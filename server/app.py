...
def _gemini_key(): return os.getenv("UCOA_GEMINI_API_KEY","").strip() or os.getenv("GEMINI_API_KEY","").strip()
def _gemini_model():
    v=os.getenv("UCOA_GEMINI_MODEL","").strip(); return v if v in _VALID_GEMINI_MODELS else _DEFAULT_GEMINI_MODEL
def _gemini_native(system,user,image=None):
    ...
    req=urllib.request.Request(f"https://generativelanguage.googleapis.com/v1beta/models/{_gemini_model()}:generateContent",data=json.dumps(payload,ensure_ascii=False).encode(),headers={"Content-Type":"application/json","x-goog-api-key":_gemini_key()},method="POST")
    ...
    return result
def _qwen_native(system,user,image=None): return app_v4_runtime._space_predict(system+"\n"+user,image)
...
def providers_probe():
    ...
    if _gemini_configured():
        try:return {"ok":True,"configured":True,"providers":[{"provider":"gemini","model":_gemini_model(),"ok":True,"vision":True}],"runtime":{"ok":True,"provider":f"gemini:{_gemini_model()}"},"response":app_v3.extract_json(_gemini_native("Return ONLY JSON.","Return exactly {\"ok\":true}."))}
    ...
    result["providers"]=rows+result.get("providers",[]); runtime=result.get("runtime",{})
    if not runtime:
        try: _,provider=provider_router.reasoning("Return ONLY JSON.","Return exactly {\"ok\":true}."); runtime={"ok":True,"provider":provider}
...
def android_plan(req:app_v3.PlanRequest,authorization:str|None=Header(default=None)):
    ...
    return _run_sync("plan",runner,req)
def android_step(req:app_v3.StepRequest,authorization:str|None=Header(default=None)):
    ...
    return _run_sync("step",runner,req)
def android_job(jid:str,authorization:str|None=Header(default=None)):
    ...
    return job
app_v3.app.routes[:]=[r for r in app_v3.app.routes if getattr(r,"path","") not in {"/v1/agent/plan","/v1/agent/step","/v1/agent/jobs/{jid}","/health"}]
...
# Put the exact /next route before the generic /{command_id} route.
_next_routes = [r for r in app_v3.app.routes if getattr(r, "path", "") == "/v1/client/commands/next"]
app_v3.app.routes[:] = _next_routes + [r for r in app_v3.app.routes if getattr(r, "path", "") != "/v1/client/commands/next"]
if not _next_routes:
