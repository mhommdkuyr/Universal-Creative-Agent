...
def wait_job(job_id:str):
    for _ in range(100):
        response=client.get(f'/v1/agent/jobs/{job_id}'); assert response.status_code==200; body=response.json()
        if body['status']=='completed': return body['result']
        if body['status']=='failed': raise AssertionError(body.get('error','job failed'))
        time.sleep(0.01)
