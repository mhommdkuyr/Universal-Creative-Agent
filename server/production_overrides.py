...
The module is loaded after the FastAPI app is initialized. In production the
handlers call Qwen3-VL directly; tests can still monkeypatch the public V3
reasoning/vision hooks without invoking the network provider.
"""
...
def _save_plan(sid: str, req: Any, result: dict[str, Any]) -> dict[str, Any]:
...
def run_plan(req: Any) -> dict[str, Any]:
    ...
    payload = json.dumps(
        {"task": req.task, "attachments": req.attachments[:8], "device": req.device, "memory": app_v3.memory(sid, 12)},
        ensure_ascii=False,
    )
    # Keep the established test seam: when reasoning is monkeypatched, use it.
    if app_v3.reasoning is not _BASE_REASONING:
        ...
        try:
            ...
            return _save_plan(sid, req, {"summary": summary, "steps": steps, "output_mode": "model", "provider": provider, "session_id": sid})
        except Exception as exc:
            return _save_plan(sid, req, {"summary": "خطة آمنة قابلة للتحقق", "steps": ["افتح الهدف المناسب.", "نفذ الإجراء المطلوب.", "تحقق من النتيجة."], "output_mode": "repair", "provider": provider, "session_id": sid, "error": str(exc)})

    last_error: Exception | None = None
    prompts = [
        app_v4_runtime.PLANNER + "\n" + payload,
        "Return ONLY one valid JSON object with keys summary and steps. steps must contain 3-5 concrete executable actions for this exact Android task, with the final action verifying the requested outcome.\nTASK:\n" + payload,
    ]
...
def run_step(req: Any) -> dict[str, Any]:
    ...
    # Compatibility mode for unit tests and explicit monkeypatches.
    reasoning_changed = app_v3.reasoning is not _BASE_REASONING
    visual_changed = app_v3.visual is not _BASE_VISUAL
    call_vision_changed = app_v3.call_vision is not _BASE_CALL_VISION
    if reasoning_changed or visual_changed or call_vision_changed:
        try:
            if call_vision_changed:
                ...
                obs, vision_provider = {"screen_summary":"compatibility without screenshot","elements":[],"confidence":0.0}, "compatibility"
            ...
            result = app_v4_runtime._normalize_action(parsed, req.screenshot_base64)
            ...
            # Preserve the old safety contract: an invalid model action must
            # complete as an observable result rather than leave a job pending.
            result = {"action":"observe","params":{},"message":"تعذر اتخاذ إجراء صالح؛ سأعيد الملاحظة بأمان.","done":False,"wait_after_ms":600,"confidence":0.0,"coordinate_space":None,"provider":"repair","vision_provider":"repair","output_mode":"compatibility_test","visual_observation":{"screen_summary":"repair","elements":[],"confidence":0.0},"error":str(exc)}
            return _save_step(sid, req, result)
