...
for _ in $(seq 1 150); do
  kill -0 "$LLAMA_PID" 2>/dev/null || { tail -n 200 "$LOG" || true; exit 1; }
  curl -fsS --max-time 2 "$UCOA_MODEL_BASE_URL/models" >/dev/null 2>&1 && { echo LOCAL_BRAIN_READY; break; }
  sleep 2
done
curl -fsS --max-time 5 "$UCOA_MODEL_BASE_URL/models" >/dev/null 2>&1 || { tail -n 200 "$LOG" || true; exit 1; }
...
JSON
curl -fsS --max-time 90 -H 'Content-Type: application/json' -X POST "$UCOA_MODEL_BASE_URL/chat/completions" --data-binary @"$ROOT/selftest-payload.json" -o "$SELFTEST"
python - "$SELFTEST" <<'PY'
import json,sys
