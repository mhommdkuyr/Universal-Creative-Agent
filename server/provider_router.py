...
def call(system, user, image=None):
    ...
    for name in _ordered(image):
        ...
        try:
            p = _provider(name)
            base, key, model, supports = _cfg(p)
            if image and not supports:
            ...
            _failure(name); errors.append(f"{name}:HTTP_{e.code}")
        except (URLError, TimeoutError) as e:
            _failure(name); errors.append(f"{name}:{type(e).__name__}")
        except Exception as e:
            _failure(name); errors.append(f"{name}:{str(e)[:120]}")
