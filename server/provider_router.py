...
PROVIDERS = [
    ...
    {"name": "deepseek", "key_env": "UCOA_DEEPSEEK_API_KEY", "base_env": None, "model_env": "UCOA_DEEPSEEK_MODEL", "default_base": "https://api.deepseek.com", "default_model": "deepseek-v4-flash-vision-exp", "vision": True},
...
def _native_gemini(model: str, key: str, system: str, user: str, image: str, timeout: float) -> str:
    url = f"https://generativelanguage.googleapis.com/v1beta/models/{quote(model, safe='')}:generateContent?key={quote(key, safe='')}"
    payload = {
        "systemInstruction": {"parts": [{"text": system}]},
        "contents": [{"role": "user", "parts": [{"text": user}, {"inlineData": {"mimeType": "image/jpeg", "data": image}}]}],
    ...
    text = "\n".join(str(part["text"]) for cand in body.get("candidates") or [] for part in (cand.get("content") or {}).get("parts") or [] if isinstance(part, dict) and part.get("text")).strip()
    if not text:
        raise RuntimeError("no Gemini content")
    return text


def _chat(base: str, key: str, model: str, system: str, user: str, image: str | None, timeout: float) -> str:
    if image and "generativelanguage.googleapis.com" in base:
        return _native_gemini(model, key, system, user, image, timeout)
    content: Any = user
    if image:
...
def _ordered(image: str | None) -> list[str]:
    ...
    for p in PROVIDERS:
        try:
            ...
            continue
    preferred = ["omniroute", "gemini", "deepseek", "cerebras", "groq"] if image else ["omniroute", "cerebras", "groq", "gemini", "deepseek"]
...
def call(system: str, user: str, image: str | None = None) -> tuple[str, str]:
    ...
    for name in _ordered(image):
        ...
        try:
            ...
            with span("ai.provider", f"{name} inference", provider=name, model=model, multimodal=bool(image and vision)):
                raw = _chat(base, key, model, system, user, image if image and vision else None, timeout)
            if not raw.strip():
...
def probe(vision: bool = False) -> dict[str, Any]:
    ...
    image: str | None = None
    if vision:
        # 1x1 white JPEG, enough to validate a multimodal request without user data.
    ...
    for p in PROVIDERS:
        ...
        try:
            ...
            if vision and not supports_vision:
                raise RuntimeError("provider does not advertise vision")
            raw = _chat(base, key, model, "Return ONLY JSON.", "Return exactly {\"ok\":true}.", image if vision else None, min(VISION_TIMEOUT if vision else TEXT_TIMEOUT, 8))
            results.append({"provider": name, "model": model, "vision": supports_vision, "ok": bool(raw.strip()), "latency_s": round(time.perf_counter() - started, 3)})
        except Exception as exc:
