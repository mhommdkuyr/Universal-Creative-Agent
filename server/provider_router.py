...
TEXT_TIMEOUT = float(os.getenv("UCOA_PROVIDER_TEXT_TIMEOUT", "12"))
VISION_TIMEOUT = float(os.getenv("UCOA_PROVIDER_VISION_TIMEOUT", "20"))
...
# Gemini is the production cloud model; the Android APK carries the local Qwen model.
PROVIDERS = [
...
_BREAKERS={p["name"]:[0,0.0] for p in PROVIDERS}; _VISION_CACHE={}
...
def _is_open(name):
    ...
    if time.monotonic()-t>=CB_COOLDOWN:_BREAKERS[name]=[0,0.0];return False
    return True
def _success(name):_BREAKERS[name]=[0,0.0]
def _failure(name):
    f,t=_BREAKERS[name];f+=1;_BREAKERS[name]=[f,time.monotonic() if f>=CB_FAILURES else t]

def _chat(base,key,model,system,user,image,timeout):
    content=user
    if image: content=[{"type":"text","text":user},{"type":"image_url","image_url":{"url":f"data:image/jpeg;base64,{image}","detail":"high"}}]
    ...
    req=Request(url,data=json.dumps(payload,ensure_ascii=False).encode(),headers={"Content-Type":"application/json","Accept":"application/json","Authorization":f"Bearer {key}"},method="POST")
...
def safe_text_probe():
    ...
    return {"ok":True,"runtime":{"ok":True,"provider":provider},"response":_extract_json(raw)}
