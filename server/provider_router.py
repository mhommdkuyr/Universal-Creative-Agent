...
def _chat(base:str,key:str,model:str,system:str,user:str,image:str|None,timeout:int)->str:
 ...
 try:
  with urlopen(req,timeout=timeout) as resp: body=json.loads(resp.read().decode())
 except HTTPError as exc: raise RuntimeError(f"HTTP_{exc.code}") from exc
 except (URLError,TimeoutError) as exc: raise RuntimeError(type(exc).__name__) from exc
...
def visual(task:str,ui_tree:str,image:str)->tuple[dict[str,Any],str]:
 system=("You are UCOA visual perception. Inspect only the screenshot and UI tree. Do not choose an action. "
         "Return ONLY valid JSON, no markdown: {\"screen_summary\":string,\"elements\":[{\"text\":string,\"role\":string,\"x\":number,\"y\":number}],\"visible_goal_state\":string,\"confidence\":number}. Never invent unseen elements.")
 prompt=json.dumps({"task":task,"ui_tree":ui_tree[:18000]},ensure_ascii=False)
 raw,provider=call(system,prompt,image)
 try: return _extract_json(raw),provider
 except ValueError:
  p=_get(provider); base,key,model,_=_cfg(p)
  repair_system="Convert RAW VISUAL ANALYSIS into ONLY valid JSON matching this schema. Preserve only facts explicitly present in RAW."
  repair_user=("SCHEMA={\"screen_summary\":string,\"elements\":[{\"text\":string,\"role\":string,\"x\":number,\"y\":number}],\"visible_goal_state\":string,\"confidence\":number}\n"
               "TASK="+task+"\nRAW="+raw[:5000])
  repaired=_chat(base,key,model,repair_system,repair_user,None,min(TIMEOUT,8))
  return _extract_json(repaired),provider
...
def probe()->dict[str,Any]:
 ...
 for p in PROVIDERS:
  ...
  try:
   ...
   results.append({"provider":name,"model":p.get("default_model"),"vision":p["vision"],"ok":False,"latency_s":round(time.perf_counter()-started,3),"error":str(exc)[:160]})
