...
def _native_gemini(model: str, key: str, system: str, user: str, image: str, timeout: int) -> str:
    ...
    payload={"system_instruction":{"parts":[{"text":system}]},"contents":[{"role":"user","parts":[{"text":user},{"inline_data":{"mime_type":"image/jpeg","data":image}}]}],"generationConfig":{"temperature":0,"maxOutputTokens":MAX_TOKENS}}
...
def _chat(base: str, key: str, model: str, system: str, user: str, image: str | None, timeout: int) -> str:
    # Gemini's native endpoint is used for multimodal calls because it avoids
    # compatibility differences in image_url handling across gateways.
...
def call(system:str,user:str,image:str|None=None)->tuple[str,str]:
    ...
    for name in _order(image):
        try:
            p=_get(name); base,key,model,vision=_cfg(p)
...
def visual(task:str,ui_tree:str,image:str)->tuple[dict[str,Any],str]:
    system=("You are UCOA visual perception. Inspect only the screenshot and UI tree. "
...
def probe()->dict[str,Any]:
    ...
    for p in PROVIDERS:
        ...
        try:
            ...
            results.append({"provider":name,"model":p.get("model_env"),"vision":p["vision"],"ok":False,"latency_s":round(time.perf_counter()-started,3),"error":type(exc).__name__})
