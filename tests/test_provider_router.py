...
def test_failover_after_error(monkeypatch):
    _configure(monkeypatch)
...
def test_circuit_breaker_skips_open_provider(monkeypatch):
    _configure(monkeypatch)
    monkeypatch.setenv("UCOA_CEREBRAS_API_KEY", "test-c")
    pr._BREAKERS["cerebras"].opened_at = __import__("time").monotonic()
...
def test_vision_cache(monkeypatch):
    _configure(monkeypatch)
    # Gemini OpenAI-compatible response is used here; the payload is a tiny image.
    image = base64.b64encode(b"fake-image").decode()
    body = {"choices": [{"message": {"content": json.dumps({"screen_summary": "ok", "elements": [], "visible_goal_state": "x", "confidence": 0.9})}}]}
